...
- **Provenance on every fact.** Every fact MUST record the full chain: session → extraction model → approval model → PR.
...
`local` mode is designed for solo/offline use. It sacrifices governance for speed. `remote` mode provides full PR governance. `hybrid` mode writes sessions directly but routes all fact changes through PRs.
...
**Manual edit always wins.** If a user manually confirms or edits a fact, it is promoted to S:5 regardless of current strength. S:5 is exclusively reserved for human-authored or human-confirmed facts.
...
facts:
  - fact_id: 01JQXYZ1234567890ABCDEF    # ULID, immutable
    ...
    applies_to: null                      # optional: {"os":"linux","env":"dev"}
    provenance:
...
| `dream_consolidation` | Derived by the Dream pipeline from other facts | Cross-session pattern extraction | Inherits from inputs |
...
+ (w_t  × task_salience)            # Ovsiankina bonus for open tasks
```
...
### `last_referenced` update rules

`last_referenced` MUST only be updated on:
...
Examples:
- `src/api/auth/middleware.ts` → `files/src__api__auth__middleware.ts.md`
- `src/api/` → `folders/src__api.md`
...
```markdown
## Facts
- [S:4|V:cor] postgres runs on port 5433 in this dev env <!-- umx:{"id":"01JQXYZ1234567890ABCDEF","conf":0.97,"cor":["aider"],"pr":"#47","src":"claude-code","ss":"2026-04-03-01JQXYZ9876543210","st":"tool_output","cr":"2026-04-03T20:11Z","lr":"2026-04-04T09:00Z","v":"corroborated","cs":"stable","sup":"01JQXABCDEF0000000000"} -->
- [S:3|V:sr] CORS warnings on /api/auth can be ignored in dev <!-- umx:{"id":"01JQXYZ2345678901BCDEFG","conf":0.88,"cor":[],"src":"aider","ss":"2026-04-01-01JQXYZ1111111111","st":"tool_output","cr":"2026-04-01T14:33Z","v":"self-reported","cs":"fragile"} -->
```
...
| Confidence | `conf` | MUST | Extractor output |
| Corroborated by | `cor` | MUST | Tool names array |
| PR reference | `pr` | SHOULD | gitmem mode |
| Source tool | `src` | MUST | Tool that produced the fact |
...
| Last referenced | `lr` | SHOULD | ISO 8601 |
...
| Applies to | `at` | MAY | Environment qualifiers JSON |
...
**Full provenance** is stored in the derived `.umx.json` file, not inline. The inline metadata carries the minimum viable set for display, quick lookup, and round-tripping through manual edits.

If a user adds a line without metadata, the parser MUST assign S:5, `verification: human-confirmed`, `source_type: user_prompt`, and generate the metadata block. If a user edits an existing line, the parser MUST detect the change via the `id`, promote to S:5, and update JSON.
...
**Size constraint:** The index section of `MEMORY.md` MUST stay under a configurable limit (default: 200 lines, configurable via `config.yaml` `memory_index.max_lines`). Enforced by Prune phase.
...
| # | Phase | Action | Output |
|---|-------|--------|--------|
| 1 | **Orient** | Read `MEMORY.md`, `CONVENTIONS.md`, and `meta/manifest.json`. Check `meta/schema_version`. Compare project file tree against existing `folders/` and `files/` entries (detect orphaned scoped memory from renames). Check tombstones. | Current memory map + orphan candidates + convention context |
...
- **Stale reference:** files/src__legacy__api__auth.md → file deleted in project repo on 2026-03-22
- **Orphan ID:** fact X corroborated_by references id 01JQXABC... which does not exist
...
- `hybrid`: local dream for immediate post-session consolidation (writes to main). Remote dream for cross-project clustering, audit, and principle promotion (writes via PR).
...
- **Hot:** `MEMORY.md`, `CONVENTIONS.md`, `manifest.json`, `principles/` — always injected at session start, roughly 2000 tokens. Maintained by Consolidate.
...
```
- Identical fact_id           → merge metadata (take higher fact_score)
...
- User-defined patterns from `config.yaml` `redaction.patterns`
...
| **`last_referenced` inflation** | Every injection counts as retrieval | Only explicit use updates `last_referenced`; silent injection does not |

---

## 23  Conformance

A tool claiming "umx-compatible" MUST satisfy these minimum requirements:
...
prune:
  ...
  memory_index_max_lines: 200         # MEMORY.md index size limit
...
sessions:
  ...
  redaction_patterns: []              # additional regex patterns
  retention:
...
weights:
  ...
  corroboration: 0.4
  verification: 0.3
  source_type: 0.4                    # ground_truth_code > tool_output > external_doc > user_prompt > llm_inference
  task_salience: 0.5
```
...
|-------|-----------|--------------|
| **0** | Foundation | Scope spec · file format · fact schema (ULID, verification, `source_type`, consolidation_status, conflicts_with, supersedes/superseded_by) · conflict format · private/secret split · schema_version · local path convention · `CONVENTIONS.md` template |
| **1** | Core library | `scope.py` + `memory.py` + `strength.py` + `identity.py` · composite scoring (strength + verification + source_type + task_salience) · AIP hook integration · session log write + redaction · project discovery + slug collision · **test harness: known sessions → expected facts at expected strengths** |
| **2** | Dream pipeline | Orient (reads CONVENTIONS.md + manifest) → Gather (source_type tagging) → Consolidate (supersession chains) → Lint (weekly: orphans, tag drift, convention violations) → Prune (abandon_days, manifest rebuild) · 3-gate + gap trigger · provider rotation · `.gitignore` safety · tombstones · consolidation_status lifecycle (3-rule transition) · schema_conflict flagging |
| **3** | Read adapters | Claude Code · Aider · generic · hybrid gather · corroboration bonus (independence rules) · SQLite FTS (incremental rebuild, source_type indexed) · **bulk import: `umx import --tool claude-code`** |
| **4** | Injection layer | Tier 1–4 hooks/shims/MCP · budget enforcement · relevance scoring (encoding context) · task salience injection (open/blocked/abandoned) · `[fragile]` marker · gap signal emission with resolution_context · mid-session scratchpad fast path · legacy bridge |
| **5** | gitmem backend | `umx init` bootstrap · GitHub org layout · push queue · PR pipeline · L1/L2/L3 governance · CONVENTIONS.md in L2 reviewer context · audit trail · `umx sync` · Actions workflow templates · Lint PR automation |
...
| **7** | Hardening | `umx merge` · arbitrator agent · `umx confirm` / `umx history` / `umx resume` / `umx meta` CLI · schema migration tooling · session compression · time decay tuning · signed commits · hypothesis branches · orphaned scope detection |
| **8** | Cross-project | Cross-project dream · promotion protocol · deep re-derivation (`umx audit --rederive`) · principle governance |
| **9** | Ecosystem | `aip mem` integration · published spec · legacy bridge · third-party adoption |
...
- **Composite score weights** — require empirical tuning. All weights exposed as config initially.
- **Time decay λ tuning** — default λ 0.023 is a starting point.
- **Extraction prompt design** — most implementation-critical piece not yet specified. Must include Bartlett schema-conflict flagging instruction.
...
- **Bi-temporal fact validity** — `valid_from` / `valid_to` fields with `umx query --as-of <date>` enables point-in-time queries ("what did we believe about the infra on January 15th?"). Was proposed for v0.7 but deferred: most coding facts are point-in-time current state, not time-ranged assertions. `expires_at` covers the common TTL case. `supersedes`/`superseded_by` chains provide implicit temporal evolution. Full bi-temporal support adds significant schema and query complexity for a narrow set of use cases. Revisit if time-sensitive facts (deploy targets, feature flags, API versions) prove problematic in practice.
- **Formal hot/warm/cold tier promotion** — Section 16 informally names the three tiers (always-loaded MEMORY.md/CONVENTIONS.md/manifest, on-demand facts/topics, rarely-touched sessions) but does not define formal promotion/demotion rules. The existing Consolidate/Prune loops handle tier movement implicitly via `usage.sqlite`-driven calibration. Formalise rules if the implicit approach misallocates budget in practice (e.g., hot facts growing unboundedly, or useful facts stuck in the warm tier).
- **Encoding depth / processing depth** — Levels of Processing (Craik & Lockhart 1972) suggests facts formed through action-outcome cycles (debugging, fixing) should carry higher initial strength than passive observations. Optional `processing_depth` field deferred to post-v1. Partially subsumed by the `source_type` enum (`tool_output` implies active use, `llm_inference` implies passive reasoning).
- **When to create project memory** — rule of thumb: if the project has architecture, it gets memory.
- **Verification field weight calibration** — how much should `corroborated` vs `self-reported` actually affect composite score?
- **Source type weight calibration** — the proposed `source_type` weights (Section 6) are estimates. Tuning should be driven by `usage.sqlite` calibration: if `llm_inference` facts are consistently accurate in practice, raise their weight; if `ground_truth_code` facts are frequently stale, lower theirs.
