...
from umx.sessions import scheduled_archive_sessions, write_session
...
def run(
    ...
    try:
        ...
        archive_result = scheduled_archive_sessions(repo_dir, config=cfg)
        ...
        elif result["archived_sessions"]:
            commit_result = git_add_and_commit(
                repo_dir,
                message="umx: archive sessions",
                config=cfg,
            )
            if commit_result.failed:
                raise RuntimeError(
