...
### Added
- **Codebase onboarding-unit drift detection** (`umx codebase-drift` + `umx doctor`). Onboarding units already recorded a `drift_hash` over the source files they describe (spec §3b), but nothing ever read it back — the hash was written and never checked, so stale units were never flagged. `check_onboarding_drift` now recomputes each unit's hash and reports units whose source files have changed or been deleted; `umx codebase-drift` exposes it directly and `umx doctor` surfaces a `codebase_drift` summary so staleness shows up in routine health checks.
...
- Contamination-aware confidence decay (`decay_confidence_by_chain_depth`) is now wired into IMX-derived candidate ingestion: candidates carrying an IMX `chain_depth` have their starting confidence decayed per handoff hop (spec §30a), since errors/bias compound across agent-to-agent chains. Recency/time decay was already covered by `strength.recency_score` (Ebbinghaus `exp(-λ·age_days)`).
