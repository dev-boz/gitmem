...
| Quarantine visibility | **R** | operator cannot tell that blocked sessions or push-safety reports exist, so risky data sits unresolved | `umx doctor` summarizes `local/quarantine/`; push safety writes JSON reports there; `umx status` now surfaces the same summary; viewer exposes a quarantine queue with masked previews and release/discard actions for blocked session files | push-safety reports still live in `status`/`doctor`, not the viewer queue; quarantine retention policy is still missing | keep status/doctor/viewer semantics aligned and decide whether push-safety reports need richer UI |
...
1. **Quarantine handling is still split across surfaces.** `status` and `doctor` now surface quarantine, and the viewer handles blocked session files, but push-safety reports still rely on the summary surfaces rather than a dedicated UI.
...
- **T2.4** — quarantine queue, release/discard workflow, local decision logging, and `status`/`doctor` surfacing are now present; push-safety reports still rely on the summary surfaces rather than the viewer
